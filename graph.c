...
char* pop() {
    ...
    char* result = copy_string(temp->data);
    stack_node = temp->next;
    free(temp);
    return result;
}

int is_stack_empty() { return stack_node == NULL; }
...
int format_expression(char* expr, char* formatted) {
    ...
    for (i = 0; expr[i]; i++) {
        if (expr[i] == 'x') {
            ...
            formatted[j++] = '^';
        } else if (expr[i] == 's' && i + 2 < len && expr[i + 1] == 'i' && expr[i + 2] == 'n') {
            formatted[j++] = 's';
            formatted[j++] = 'i';
            formatted[j++] = 'n';
            i += 2;
        } else if (expr[i] == 'c' && i + 2 < len && expr[i + 1] == 'o' && expr[i + 2] == 's') {
            formatted[j++] = 'c';
            formatted[j++] = 'o';
            formatted[j++] = 's';
            i += 2;
        } else if (expr[i] == 't' && i + 2 < len && expr[i + 1] == 'a' && expr[i + 2] == 'n') {
            formatted[j++] = 't';
            formatted[j++] = 'a';
            formatted[j++] = 'n';
            i += 2;
        } else if (expr[i] == 'c' && i + 2 < len && expr[i + 1] == 't' && expr[i + 2] == 'g') {
            formatted[j++] = 'c';
            formatted[j++] = 't';
            formatted[j++] = 'g';
            i += 2;
        } else if (expr[i] == 's' && i + 3 < len && expr[i + 1] == 'q' && expr[i + 2] == 'r' &&
                   expr[i + 3] == 't') {
            formatted[j++] = 's';
            formatted[j++] = 'q';
            formatted[j++] = 'r';
            formatted[j++] = 't';
            i += 3;
        } else if (expr[i] == 'l' && i + 1 < len && expr[i + 1] == 'n') {
            formatted[j++] = 'l';
            formatted[j++] = 'n';
            i += 1;
        } else if ((expr[i] >= '0' && expr[i] <= '9') || expr[i] == '.' || expr[i] == '+' || expr[i] == '*' ||
...
int to_rpn(char* input, char* output) {
    ...
    for (i = 0; input[i]; i++) {
        ...
        if ((input[i] >= '0' && input[i] <= '9') || input[i] == '.') {
            ...
            while (!is_stack_empty() && precedence(stack_node->data[0]) >= precedence(input[i]) &&
                   stack_node->data[0] != '(') {
                char* op = pop();
                strcpy(output + j, op);
                j += strlen(op);
            ...
            while (!is_stack_empty() && stack_node->data[0] != '(') {
                char* op = pop();
                strcpy(output + j, op);
                j += strlen(op);
                output[j++] = ' ';
            ...
            if (!is_stack_empty() && stack_node->data[0] == '(') {
                char* op = pop();
                free(op);
            }
        } else if (i + 2 < len && input[i] == 's' && input[i + 1] == 'i' && input[i + 2] == 'n') {
            push("sin");
            i += 2;
        } else if (i + 2 < len && input[i] == 'c' && input[i + 1] == 'o' && input[i + 2] == 's') {
            push("cos");
            i += 2;
        } else if (i + 2 < len && input[i] == 't' && input[i + 1] == 'a' && input[i + 2] == 'n') {
            push("tan");
            i += 2;
        } else if (i + 2 < len && input[i] == 'c' && input[i + 1] == 't' && input[i + 2] == 'g') {
            push("ctg");
            i += 2;
        } else if (i + 3 < len && input[i] == 's' && input[i + 1] == 'q' && input[i + 2] == 'r' &&
                   input[i + 3] == 't') {
            push("sqrt");
            i += 3;
        } else if (i + 1 < len && input[i] == 'l' && input[i + 1] == 'n') {
            push("ln");
            i += 1;
        } else {
            return 0;
...
int round_value(double value) { return (int)(value + 0.5); }
